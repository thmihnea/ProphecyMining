...
public class RegionCommandHandler implements CommandExecutor {
    ...
    public RegionCommandHandler() {
        Objects.requireNonNull(ProphecyMining.getInstance().getCommand("sbregion")).setExecutor(this);
    }

    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] args) {
        if (args.length == 0) {
            LangUtil.sendStringList(commandSender, Settings.LANG_REGION_HELP_MESSAGE);
            return false;
        }
        String subCommand = args[0].toLowerCase(); subCommand = subCommand.substring(0, 1).toUpperCase() + subCommand.substring(1);
        try {
            Class<?> clazz = Class.forName("by.thmihnea.prophecymining.region.command.Command" + subCommand);
