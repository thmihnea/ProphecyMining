...
public class RegionBootstrapper {
    ...
    public static void setupSavedRegions() {
        ...
        Arrays.stream(regionDirectory.listFiles()).forEach(file -> {
            ...
            ProphecyMining.getInstance().logInfo("Found region file " + file + " in the region directory. Attempting to load it.");
    ...
    public static void setupBlocks(FileConfiguration cfg, World world, CuboidRegion cuboidRegion) {
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(ProphecyMining.getInstance(), () -> System.out.println(cuboidRegion.getOres()), 5L);
    }
}
